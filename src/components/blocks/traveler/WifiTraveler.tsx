...
export function WifiTraveler({ title, data }: { title?: string; data: any; ctx: any; visibility?: any }) {
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        <div className="grid gap-2 text-sm">
          <div><span className="font-medium">Réseau :</span> {ssid || "—"}</div>
          <div><span className="font-medium">Mot de passe :</span> {pass || "—"}</div>
        </div>

        <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3 text-xs text-zinc-700">
          <div className="font-medium text-zinc-900">QR payload (starter)</div>
          <div className="mt-1 break-all">{ssid && pass ? wifiQrString(ssid, pass) : "—"}</div>
        </div>
